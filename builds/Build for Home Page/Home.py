...
def run_other_script():
    ...
    print(data)
    window.destroy()
    subprocess.run([sys.executable, 'Auth1.py',data])
    
def run_login():
    data2 = entry_1.get()
    window.destroy()
    subprocess.run([sys.executable, 'Login.py',data2])
  


OUTPUT_PATH = Path(__file__).parent
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Academics\Chat Application\build\assets\frame0")
